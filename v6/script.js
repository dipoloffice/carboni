...
const alignedMegaNav = document.querySelector(".primary-nav--aligned-mega");
const alignedMegaPanel = document.querySelector(".aligned-mega-panel");

function syncAlignedMegaColumns() {
  if (!alignedMegaNav || !alignedMegaPanel) return;

  const panelInner = alignedMegaPanel.querySelector(".aligned-mega-inner");
  if (!panelInner) return;

  const panelRect = panelInner.getBoundingClientRect();
  alignedMegaNav.querySelectorAll("[data-mega-target]").forEach((trigger) => {
    const group = alignedMegaPanel.querySelector(`[data-mega-group="${trigger.dataset.megaTarget}"]`);
    if (!group) return;

    const triggerRect = trigger.getBoundingClientRect();
    group.style.setProperty("--mega-group-left", `${Math.round(triggerRect.left - panelRect.left)}px`);
    group.style.setProperty("--mega-group-width", `${Math.round(triggerRect.width)}px`);
  });
}

if (alignedMegaNav && alignedMegaPanel) {
  syncAlignedMegaColumns();
  window.addEventListener("resize", syncAlignedMegaColumns);
  window.addEventListener("load", syncAlignedMegaColumns);
}
...
const sectionObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      ...
      entry.target.classList.remove('is-in-view'); // Replay animation when scrolling back
    }
  });
}, { threshold: 0.15 });

document.querySelectorAll('.snap-page > section').forEach(sec => {
